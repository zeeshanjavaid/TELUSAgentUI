...
public class CollectionDataService {
    ...
    public List<LookUpResponseWithTeamName> getEntitySearch(@RequestParam(required = true)String inputType, @RequestParam(required = true)String inputValue, @RequestParam(required = true)String level, @RequestParam(required = true)String searchMatchCriteria, @RequestParam(required = true)String billingSystem, Integer offset, Integer limit) throws Exception  {
        ...
        if (isStubEnabled) {
                ...
                return  setTeamNameBasedOnEmplId(entitySearchResponseArray,0);
       ...
       //    return objectMapper.readValue(responseStr,EntitySearchResponseArray.class);
           		 return  setTeamNameBasedOnEmplId(objectMapper.readValue(result,EntitySearchResponseArray.class),Integer.parseInt(totalNoOfElement));
...
//entityDetails
 ...
 if (isStubEnabled) {
    ...
    public List<TeamsActionViewResponseWIthTotalCount> getActionViewByTeam(String assignedAgent, String assignedTeam,String entityOwner, Date fromDueDate, Date toDueDate, String actionType, String status, String workCategory, String viewType, Integer offset, Integer limit) throws Exception {
    	...
    	//	String responseStr = telusAPIConnectivityService.executeTelusAPI(null, endPointString, HttpMethod.GET, entitySvcAuthScope);
			...
			String totalNoOfElement=headers1.getFirst("x-total-count");
    		 ...
    		 if(!StringUtils.isEmpty(result)) {
    			 teamsActionViewResponseList = objectMapper.readValue(result,
      				objectMapper.getTypeFactory().constructCollectionType(List.class, TeamsActionViewResponse.class));
    		 }
   ...
   private List<LookUpResponseWithTeamName> setTeamNameBasedOnEmplId(EntitySearchResponseArray entitySearchResponseArray,Integer totalNoOfElement)
     	{
	    	...
	    	if(!entitySearchResponseArray.isEmpty())
	    	{
		    	for(EntitySearchResponse entitySearchResponse:entitySearchResponseArray)
		    	{
			    	...
			    	lookUpResponseWithTeamName.setTotalNumberOfElement(totalNoOfElement);
	...
	for(TeamsActionViewResponse teamsActionViewResponse:teamsActionViewResponseList)
		{
			...
			teamsActionViewResponseWIthTotalCount.setEntityId(teamsActionViewResponse.getEntityId());
			teamsActionViewResponseWIthTotalCount.setEntityName(teamsActionViewResponse.getEntityName());
			teamsActionViewResponseWIthTotalCount.setEntityOwner(commonUtilityService.getNameUsingEmpId(teamsActionViewResponse.getEntityOwner()));
			teamsActionViewResponseWIthTotalCount.setEntityType(teamsActionViewResponse.getEntityType());
			teamsActionViewResponseWIthTotalCount.setCollectionStatus(teamsActionViewResponse.getCollectionStatus());
			teamsActionViewResponseWIthTotalCount.setActionType(teamsActionViewResponse.getActionType());
			teamsActionViewResponseWIthTotalCount.setPriority(teamsActionViewResponse.getPriority());
			teamsActionViewResponseWIthTotalCount.setDueDate(teamsActionViewResponse.getDueDate());
			teamsActionViewResponseWIthTotalCount.setStatus(teamsActionViewResponse.getStatus());
		...
		return teamsActionViewResponseWIthTotalCounts;
	}
}