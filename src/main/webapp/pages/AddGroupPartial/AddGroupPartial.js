    ...
    if (Partial.Widgets.GroupNameText.datavalue == undefined) {
        ...
        if (Partial.pageParams.groupId) {
            Partial.Variables.getAllGroups.dataSet.forEach(function(group) {
                if (group.name.toLowerCase() == Partial.Widgets.GroupNameText.datavalue.toLowerCase() && group.id != Partial.pageParams.groupId) {
                    Partial.groupNameExists = true;
                }
            });
        } else {
            Partial.Variables.getAllGroups.dataSet.forEach(function(group) {
                if (group.name.toLowerCase() == Partial.Widgets.GroupNameText.datavalue.toLowerCase()) {
        ...
        if (!Partial.groupExists && !Partial.groupNameExists) {
            if (Partial.pageParams.groupId && Partial.pageParams.groupId > 0) {
                Partial.Variables.updateGroup.setInput({
                    'id': Partial.pageParams.groupId,
                    'name': Partial.Variables.groupData.dataSet.GroupName,
                    ...
                    'active': true,
                    'createdBy': Partial.Variables.getGroup.dataSet[0].createdBy,
                    'createdOn': Partial.Variables.getGroup.dataSet[0].createdOn,
                    'updatedBy': App.Variables.getLoggedInUserId.dataSet[0].id,
                    'updatedOn': new Date()
                });

                Partial.Variables.updateGroup.update();
            } else {
...
Partial.createGrouponSuccess = function(variable, data) {
    ...
    /*else {
           ...
           Partial.Variables.uploadGroupItem.dataSet.roles.forEach(function(role) {
               Partial.Variables.CreateGroupRole.setInput({
                   'groupId': data.id,
                   'roleId': role.id
               });
           ...
           Partial.Variables.uploadGroupItem.dataSet.users.forEach(function(usr) {
               Partial.Variables.CreateGroupUser.setInput({
                   'groupId': data.id,
                   'userId': usr.id
               });
