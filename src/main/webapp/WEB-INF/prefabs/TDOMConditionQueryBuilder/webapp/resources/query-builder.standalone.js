...
(function(root, factory) {
    ...
    QueryBuilder.types = {
        ...
        'array': 'array',
        'number': 'number',
        'dupcheck.string': 'dupcheck.string',
        'dupcheck.number': 'dupcheck.number',
        'dupcheck.date': 'dupcheck.date',
        'dupcheck.boolean': 'dupcheck.boolean',
        'dupcheck.currency': 'dupcheck.currency',
    };
    ...
    QueryBuilder.OPERATORS = {
        ...
        'is exactly same': {
            type: 'is exactly same',
            nb_inputs: 0,
            multiple: false,
            apply_to: ['dupcheck.string', 'dupcheck.number', 'dupcheck.date', 'dupcheck.currency', 'dupcheck.boolean']
        },
        'check within days': {
            type: 'check within days',
            nb_inputs: 1,
            multiple: false,
            apply_to: ['dupcheck.date']
        },
        'dupcheck_contains': {
            type: 'dupcheck_contains',
            nb_inputs: 0,
            multiple: false,
            apply_to: ['dupcheck.string']
        },
        'matched first X characters': {
            type: 'matched first X characters',
            nb_inputs: 1,
            multiple: false,
            apply_to: ['dupcheck.string']
        },
        'matched last X characters': {
            type: 'matched last X characters',
            nb_inputs: 1,
            multiple: false,
            apply_to: ['dupcheck.string']
        },
    ...
    QueryBuilder.DEFAULTS = {
        ...
        icons: {
            ...
            remove_group: 'fico-icon-minus-circle fa-lg',
            remove_rule: 'fico-icon-minus-circle fa-2x',
    ...
    QueryBuilder.prototype.checkFilters = function(filters) {
        ...
        filters.forEach(function(filter, i) {
            ...
            if (!filter.input) {
                // FICO: The input value of dupcheck date and dupcheck string type is number
                filter.input = (QueryBuilder.types[filter.type] === 'number' ||
                    QueryBuilder.types[filter.type] === 'dupcheck.string' ||
                    QueryBuilder.types[filter.type] === 'dupcheck.date') ? 'number' : 'text';
    ...
    QueryBuilder.prototype.createRuleOperators = function(rule) {
        ...
        // FICO: Sort the operators alphabetically
                    ...
                    for (var j = 0; j < tempValue.length; j++) {
                        switch (QueryBuilder.types[filter.type]) {
                            case 'string':
                                ...
                                break;
                            case 'array':
                                // FICO: If the value is not selected for array (STATIC_LIST), then show error
                                var selectedItems = rule.$el.find('.rule-value-container [name$=_0]').selectize()[0].selectize.items;
                                if (!selectedItems || selectedItems.length === 0) {
                                    var msg = "'" + filter.label + "'" + this.translate('errors', 'condition_value_required');
                                    result = [msg];
                                }
                                break;
                            case 'number':
                            case 'dupcheck.number':
                            case 'dupcheck.string':
                            case 'dupcheck.date':
                                if (tempValue[j] === undefined || tempValue[j].length === 0) {
                                    if (!validation.allow_empty_value) {
                                        // result = ['number_nan'];
                                        var msg = "'" + filter.label + "'" + this.translate('errors', 'invalid_condition_value');
                                        result = [msg];
                                    }
                                    break;
                                }
                                ...
                                if (validation.format) {
                                    ...
                                    if (!validation.format.test(tempValue[j])) {
                                        ...
                                        if (result && result.length > 0) {
                                            result[0] = filter.label + ': ' + result[0];
                                        }
                                        break;
                                    }
                                }
...
<div id="{{= it.group_id }}" class="rules-group-container"> \
  <div class="rules-group-header"> \
    <div class="btn-group pull-right group-actions"> \
      ...
      {{?}} \
      {{? it.level>1 }} \
        <div class="display-inline-block" data-delete="group"> \
...
<div id="{{= it.rule_id }}" class="rule-container"> \
  <div class="rule-header"> \
    <div class="btn-group pull-right rule-actions"> \
      <div class="display-inline-block" data-delete="rule"> \
  ...
  <label for="' + id + '">' + val + '</label> \
    ...
    QueryBuilder.regional['en'] = {
        ...
        "operators": {
            ...
            "is exactly same": "is exactly same",
            "check within days": "check within days",
            "dupcheck_contains": "contains",
            "matched first X characters": "matched first X characters",
            "matched last X characters": "matched last X characters",
            "is in": "is in",
            "is not in": "is not in",
            "between": "between"
        },
        "errors": {
            ...
            // FICO added error messages for i18n
            "invalid_condition_value": " condition value is not valid. Enter a value with numbers. ",
            "condition_value_required": " condition value is required. Select a value. "
        },
        "invert": "Invert",
        "NOT": "NOT"
    };
    ...
    QueryBuilder.regional['es'] = {
        ...
        "operators": {
            ...
            // custom by FICO
            "is equal to": "igual",
            "is not equal to": "distinto",
            "is less than": "menor",
            "is less than or equal to": "menor o igual",
            "is greater than": "mayor",
            "is greater than or equal to": "mayor o igual",
            "is empty": "is empty",
            "is not empty": "is not empty",
            "true": "true",
            "false": "false",
        ...
        "errors": {
            ...
            // FICO added error messages for i18n
            "invalid_condition_value": " condition value is not valid. Enter a value with numbers. (for es lang) ",
            "condition_value_required": " condition value is required. Select a value. (for es lang)"
        }
    };
